...
class OrganizationDashboard:
    ...
    def render(self):
        ...
        with col1:
            if st.button("🚀 고속 조직 분석 실행 (멀티프로세싱)", type="primary", use_container_width=True):
                self.execute_organization_analysis(org_id, org_level, date_range, use_singleton=False)
        with col2:
            if st.button("🔵 싱글톤 조직 분석 실행", type="secondary", use_container_width=True):
                self.execute_organization_analysis(org_id, org_level, date_range, use_singleton=True)
    
    def execute_organization_analysis(self, org_id: str, org_level: str, date_range: tuple, use_singleton: bool = False):
        ...
        Args:
            ...
            org_level: 조직 레벨
            date_range: 분석 날짜 범위
            use_singleton: 싱글톤 모드 사용 여부
        """
        
        # 모드에 따른 메시지 표시
        if use_singleton:
            st.info("🔵 싱글톤 모드로 실행합니다. (데이터 정확성 우선)")
        else:
            st.info("🚀 고속 배치 처리 모드로 실행합니다. (속도 우선)")
        ...
        with st.spinner("조직 분석 중..."):
            try:
                ...
                # 싱글톤 모드 여부에 따라 배치 프로세서 선택
                if use_singleton:
                    # 싱글톤 배치 프로세서 사용
                    from src.analysis.singleton_batch_processor import SingletonBatchProcessor
                    batch_processor = SingletonBatchProcessor()
                    # employees는 문자열 리스트이므로 올바르게 필터링
                    batch_processor.org_data = batch_processor.org_data[
                        batch_processor.org_data['사번'].astype(str).isin(employees)
                    ]
                else:
                    # 멀티프로세싱 배치 프로세서 사용
                    try:
                ...
                while current_date <= end_dt:
                        ...
                        if use_singleton:
                            # 싱글톤 모드: 순차 처리
                            batch_results = []
                            for emp_id in valid_employees_for_date:
                                try:
                                    # 직원 이름 가져오기
                                    emp_name = ""
                                    org_data = batch_processor.org_data
                                    emp_row = org_data[org_data['사번'].astype(str) == str(emp_id)]
                                    if not emp_row.empty:
                                        emp_name = emp_row.iloc[0]['성명']
                                    
                                    result = batch_processor._analyze_single_employee_date(
                                        str(emp_id), 
                                        emp_name, 
                                        current_date, 
                                        save_to_db=False
                                    ...
                                    if result:
                                        ...
                                        # 디버깅: 싱글톤 결과 확인
                                        if len(batch_results) == 0:  # 첫 번째 결과만
                                            self.logger.info(f"[싱글톤 디버깅] result keys: {list(result.keys())}")
                                            if 'activity_analysis' in result:
                                                self.logger.info(f"[싱글톤 디버깅] activity_analysis: {result['activity_analysis']}")
                                            else:
                                                self.logger.warning("[싱글톤 디버깅] activity_analysis가 없습니다!")
                                        batch_results.append(result)
                                except Exception as e:
                                    import traceback
                                    self.logger.error(f"싱글톤 분석 오류 {emp_id}: {e}")
                                    self.logger.error(traceback.format_exc())
                        else:
                            # 멀티프로세싱 모드
                            # 진행 상황 표시
                            mp_status = st.empty()
                            mp_status.info(f"🚀 멀티프로세싱 모드로 {current_date} 분석 중... (직원: {len(valid_employees_for_date)}명, 워커: 4개)")
                            
                            # 진행 상황 콜백 정의 (현재는 사용하지 않음)
                            # def update_progress(completed, total, message):
                            #     mp_status.info(f"🚀 {current_date}: {message}")
                            
                            batch_results = batch_processor.batch_analyze_employees(
                                valid_employees_for_date, 
                                current_date
                                # progress_callback=update_progress  # 임시로 비활성화
                            )
                            
                            # 분석 완료 메시지
                            success_count = sum(1 for r in batch_results if r.get('status') == 'success')
                            if success_count > 0:
                                mp_status.success(f"✅ {current_date}: {success_count}/{len(valid_employees_for_date)}명 분석 완료")
                            else:
                                mp_status.warning(f"⚠️ {current_date}: 분석 성공 0명")
                        ...
                        if use_singleton:
                            ...
                            for result in batch_results:
                                if result.get('status') == 'success':
                                    try:
                                        batch_processor.result_saver.save_individual_analysis(result)
                                        saved_count += 1
                                    except Exception as e:
